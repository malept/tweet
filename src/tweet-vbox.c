  ...
  if (priv->in_press)
    {
...

      {
        ...
        old_y = geometry.y + TWEET_CANVAS_PADDING;
        new_y = 100 + TWEET_CANVAS_PADDING;
...
tweet_vbox_constructed (GObject *gobject)
{
  ...
  clutter_actor_set_position (priv->spinner,
                              (canvas_req.width + (2 * TWEET_CANVAS_PADDING) - 128) / 2,
                         ...
                         TweetVBox     *vbox)
{
  ...
  clutter_actor_set_position (priv->spinner,
                              (allocation->width + (2 * TWEET_CANVAS_PADDING) - 128) / 2,
...
tweet_vbox_init (TweetVBox *vbox)
{
  ...
  tweet_canvas_set_border_width (TWEET_CANVAS (priv->canvas), TWEET_CANVAS_PADDING);
