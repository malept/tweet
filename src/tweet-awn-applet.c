  ...
  switch (event->button)
  {
    ...
    case 3: /* {{{ (right mouse button) */
      ...
      if (!tweet->menu)
      {
        ...
        g_signal_connect (G_OBJECT (item), "activate", G_CALLBACK (tweet_applet_menu_show_about), NULL);
...
awn_applet_factory_initp (const gchar *uid, gint orient, gint height)
{
  ...
  awn_applet_simple_set_temp_icon (tweet->applet,
                                   gtk_icon_theme_load_icon (theme,
                                                             "document-send",
                                                             height, 0, NULL));
  g_signal_connect (G_OBJECT (tweet->applet), "button-press-event", G_CALLBACK (tweet_applet_onclick), tweet);
